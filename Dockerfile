...
RUN yum -y install gcc gcc-c++ kernel-devel make autoconf wget automake libtool which \
...
RUN cd /tmp && git clone --depth=1 https://github.com/apache/hadoop.git -b trunk && \
   cd /tmp/hadoop && mvn package dependency:go-offline -DskipTests=true -f pom.ozone.xml && \
